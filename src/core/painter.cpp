...
#define BTK_RESOURCE_INTERFACE_IMPL(type) \
    ...
    auto type::bind_device_resource(void *key, PaintResource *resource) -> void { \
        if (priv) return priv->add_resource(key, resource);                       \
        return;                                                                   \
    }                                                                             \
    auto type::query_device_resource(void *key) -> PaintResource              * { \
